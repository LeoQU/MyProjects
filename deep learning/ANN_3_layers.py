...
num_layer_2 = 100 # number of neures in the hidden layer 2
num_layer_3 = 20  # number of neures in the hidden layer 3
...
with tf.Graph().as_default() as graph:
  ...
  weights_hidden_1 = tf.Variable( tf.truncated_normal([image_size * image_size, num_layer_1]) )
  biases_hidden_1 = tf.Variable( tf.constant(0.1, shape=[num_layer_1]) )

  # weights and biases in the hidden layer 2
  weights_hidden_2 = tf.Variable( tf.truncated_normal([num_layer_1, num_layer_2]) )
  biases_hidden_2 = tf.Variable( tf.constant(0.1, shape=[num_layer_2]) )

  # weights and biases in the hidden layer 3
  weights_hidden_3 = tf.Variable( tf.truncated_normal([num_layer_2, num_layer_3]) )
  biases_hidden_3 = tf.Variable( tf.constant(0.1, shape=[num_layer_3]) )

  # weights and biases in the final layer
  weights_final = tf.Variable( tf.truncated_normal([num_layer_3, num_labels]) )
  ...
  hidden_layer_3 = tf.nn.relu( tf.matmul(hidden_layer_2, weights_hidden_3) + biases_hidden_3 )

  keep_prob = tf.placeholder(tf.float32)
  hidden_layer_dropout = tf.nn.dropout(hidden_layer_3, keep_prob) # dropout

  y = tf.matmul(hidden_layer_dropout, weights_final) + biases_final

  # loss function
  weights = [weights_hidden_1, weights_hidden_2, weights_hidden_3, weights_final]
